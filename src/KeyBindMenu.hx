...
class KeyBindMenu extends FlxSubState{
	...
	var keyText:Array<String> = [ "LEFT", "DOWN", "UP", "RIGHT", "PAUSE", "RESET", "MUTE", "VOLUME UP", "VOLUME DOWN", "FULLSCREEN" ];
	...
	override function create(){
		for (i in 0...keys.length){
			var k = keys[i];
			if (k == null) keys[i] = defaultKeys[i];
		}
		for (i in 0...gpKeys.length){
			var k = gpKeys[i];
			if (k == null) gpKeys[i] = defaultGpKeys[i];
		}
