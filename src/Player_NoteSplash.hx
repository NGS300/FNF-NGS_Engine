...
class Player_NoteSplash extends FlxSprite{
	public function new(XPos:Float, YPos:Float, ?Note:Int){
		super(XPos, YPos);
		LoadPlayerSplasher();
		setupPlayerSplash(XPos, YPos, Note);
	}
	public function LoadPlayerSplasher(){ // CUSTOM SONG NOTESPLASH LOAD
		switch (PlayState.SONG.song.toLowerCase()){
			default: SplashSkin();
		}
	}
	public function setupPlayerSplash(x:Float, y:Float, ?Note:Int){ // This is to Fix Splash Position
	...
	public function SplashSkin(?SplashName:String){ // NOTESPLASH TYPE TO CREATE
		switch (SplashName){
			default: addSplosh(Options.Option.JsonOptions.UseNewNoteSytle ?
					'Player_NewStyleSplash' : 'Player_Splash',
	...
	function addSplosh(Frame:String, AnimName:Array<String>, Prefix:Array<String>, ?FrameRate:Array<Int> = null, ?Force:Array<Bool> = null){ // Just Create Splash Skin in This Function
		if (Frame != null && Prefix != null){
			frames = Paths.getSparrowAtlas('note/splash/player/$Frame');
	...
	function playSplash(Note:Int, RamdomNum:Int){
		alpha = 0.6;
		animation.play("note" + Note + "-" + RamdomNum, true);
