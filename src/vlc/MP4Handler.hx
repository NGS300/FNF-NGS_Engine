...
class MP4Handler extends VlcBitmap{
	...
	function update(e:Event){
		if ((FlxG.keys.justPressed.ENTER || FlxG.keys.justPressed.SPACE) && isPlaying) finishVideo();
	...
	public function finishVideo(){
		if (FlxG.sound.music != null && pauseMusic) FlxG.sound.music.resume();
	...
	public function playVideo(path:String, ?repeat:Bool = false, pauseMusic:Bool = false){
		...
		if (FlxG.sound.music != null && pauseMusic) FlxG.sound.music.pause();
		#if sys
		play(checkFile(path));
